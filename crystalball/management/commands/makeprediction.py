    ...
    def handle(self, *args, **options):
        ...
        for ticker in ["GRMN", "AAPL", "MSFT", "GOOGL", "AMZN", "META", "TSLA", "NVDA", "INTC", "CSCO", "CAT"]:
            ...
            headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key), 
                       'azureml-model-deployment': 'cyanchickenkrtl58-1' }
            ...
            min_prediction = result[0]

            headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key), 'azureml-model-deployment': 'frankyaksfnkkpj59-1' }
            # headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key)}
            req = urllib.request.Request(url, body, headers)
            try:
                response = urllib.request.urlopen(req)
            ...
            result = result.decode("utf-8")
            result = json.loads(result)
            max_prediction = result[0]


            Prediction.objects.create(
                date = datetime.datetime.strptime(last_row['date'], '%Y-%m-%d').date(),
                ticker = ticker,
                prediction_high = max_prediction,
                prediction_low = min_prediction,
